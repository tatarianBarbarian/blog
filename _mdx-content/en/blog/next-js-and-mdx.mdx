...
- substitute default HTML elements to custom ones. For example swap `<h1>` with `<CustomH1Heading>`
...
`import`/`export` don't work in next-mdx-remote. If we want to use some components from our codebase,
we should provide them through "components" property of `<MDXRemote>` component.
